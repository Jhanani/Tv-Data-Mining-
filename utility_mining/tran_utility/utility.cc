...
void main_proc()
{
    // PK Start
    FILE *fp;
    fp = fopen("./frequent_utility_itemset.txt", "w");
    // PK End
...
//   fprintf(summary, "Database %s utility_threshold %f num_trans %d min_utility %d thr %d\n", transaction_file_name, MIN_UTILITY_PER, num_trans,MIN_UTILITY, threshold);
   ...
   for(i=0; i < maxitem-1; i++){
      for (j=0; j < cnt_ary[i].count; j++){
        //PK//printf("PK Item: %d, Item2 : %d, Utility: %f\n", i, cnt_ary[i].t2[j].item2, cnt_ary[i].t2[j].t_utility);
         if (cnt_ary[i].t2[j].t_utility >= MIN_UTILITY){
            ...
            //printf("PK Item: %d, Item2 : %d, Utility: %f\n", i, cnt_ary[i].t2[j].item2, cnt_ary[i].t2[j].t_utility);
   ...
   //PK
   // Till here, you created 2-item sets and appended each set to the Largelist
...
for (int j=0; j <  Largelist->numitems(); j++, L1iter = L1iter->next()){
  ...
  //printf("\nTotal UTILITY : %f\n", MIN_UTILITY/MIN_UTILITY_PER);

  // PK START 
  //printf("PK: k = %d, ", 2);
  for (int pp =0; pp<2; pp++)
    //printf("PK: ITEM: %d", temp->item(pp));
    fprintf(fp, "%d, ", temp->item(pp));
  //printf(" t_utility: %f\n", temp->get_t_utility());
  fprintf(fp, "%f,%f\n",temp->get_t_utility(), temp->get_t_utility()/(MIN_UTILITY/MIN_UTILITY_PER));
  // PK END
